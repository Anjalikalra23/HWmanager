...
# Try-safe imports (helps Streamlit Cloud deployment)
try:
    from langchain_community.vectorstores import FAISS
    from langchain_openai import OpenAIEmbeddings
except ModuleNotFoundError:
    st.error("⚠️ Missing dependencies. Add `langchain-openai` and `langchain-community` to requirements.txt.")
    st.stop()
...
def run():
    ...
    @st.cache_resource
    def load_or_build_vector_db(texts):
        embeddings = OpenAIEmbeddings(model="text-embedding-3-small", api_key=openai_key)
        if os.path.exists(VECTOR_DB_PATH):
            st.success("✅ Loaded existing HW7 vector database.")
            return FAISS.load_local(VECTOR_DB_PATH, embeddings, allow_dangerous_deserialization=True)
        st.info("⚙️ Building FAISS vector database for the first time...")
        all_embeddings = []
        batch_size = 50
        ...
        db = FAISS.from_embeddings(
            text_embeddings=list(zip(texts, np.array(all_embeddings))),
    ...
    # Load data & FAISS DB
    texts = load_csv_data()
    vector_db = load_or_build_vector_db(texts)
